...
export default function WorkspaceSwitcher({ collapsed }) {
  ...
  var badge = isAll ? 'ALL' : (active ? (active.slug || active.name).substring(0, 4).toUpperCase() : '—');
  ...
  return (
    <div ref={boxRef} className="relative px-2 py-2">
      ...
      >
        <span
          className="flex items-center justify-center w-7 h-7 rounded-lg flex-shrink-0 font-mono text-[9px] font-bold"
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 min-w-0">
                  ...
                  <span className="block text-[10px] text-crm-muted truncate">{ws.slug || ws.id}</span>
                </span>
