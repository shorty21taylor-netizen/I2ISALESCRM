...
import { Building2, DollarSign, Wallet, Percent, Check, Layers, RefreshCw } from 'lucide-react';
...
export default function OwnerPage() {
  ...
  var load = useCallback(function() {
    ...
    fetch('/api/owner-dashboard?start=' + p.start + '&end=' + p.end)
      ...
      .then(function(d) {
        ...
        setData(d);
        setError(null);
        // Default every offer to on, preserving any toggles already made.
  ...
  // Cash the rollup could not attribute to any offer (EOD cash with no deals in range).
  ...
  return (
    <div className="px-4 md:px-6 py-4 md:py-6 space-y-4 md:space-y-6 max-w-[1600px] mx-auto bg-orbs relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 stagger-1 relative z-10">
        <div>
          ...
          <p className="text-xs md:text-sm text-crm-muted mt-1">
            Every offer you manage, across every workspace
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={'px-3 py-1.5 rounded-lg text-xs font-mono transition-all ' +
                  (range === r.key
                    ? 'bg-crm-accent/15 text-crm-text-bright border border-crm-accent/30'
      ...
      {error && (
        <div className="glass-card p-4 text-sm text-crm-negative">Could not load owner data: {error}</div>
      )}

      {/* Combined totals across the selected offers */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 stagger-2 relative z-10">
      ...
      {unattributed > 0 && (
        <p className="text-[11px] text-crm-muted relative z-10">
          {formatCurrency(unattributed)} of EOD cash in this range could not be attributed to a specific offer
          (cash was reported with no closed deals to apportion it against) and is excluded from the totals above.
        </p>
      )}

      {/* Offer toggles */}
      <div className="glass-card overflow-hidden stagger-3 relative z-10">
        ...
        <div className="p-3 md:p-5 space-y-2">
          {offers.length === 0 && !loading && (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-crm-muted">No offers configured yet</p>
              <p className="text-xs text-crm-muted/50 mt-1">Add offers to a workspace in Settings</p>
            </div>
          )}
          ...
          {offers.map(function(o) {
            ...
            return (
              <div
                key={o.key}
                className={'glass-surface p-3 md:p-4 transition-opacity ' + (on ? '' : 'opacity-40')}
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    <div className={'absolute top-0.5 w-4 h-4 rounded-full transition-transform duration-300 ' + (on ? 'translate-x-5' : 'translate-x-0.5')}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="section-tag">{o.workspaceShortName}</span>
                      {o.undeclared && <span className="badge-warning">unlisted</span>}
                    </div>
                    <div className="text-[10px] md:text-xs text-crm-muted font-mono mt-0.5">
                      {formatNumber(o.deals)} deals &middot; {formatNumber(o.booked)} booked &middot; {Math.round(o.commissionRate * 1000) / 10}% rate
                    </div>
                  </div>
      ...
      {/* Per-company rollup */}
      <div className="glass-card overflow-hidden stagger-4 relative z-10">
        <div className="section-header">
              ...
              {byCompany.map(function(c) {
                return (
                  <tr key={c.company.workspaceId}>
                    <td>
                      <div className="text-sm font-medium text-crm-text-bright">{c.company.name}</div>
                      <div className="text-[10px] font-mono text-crm-muted">{Math.round((c.company.commissionRate || 0) * 1000) / 10}% commission</div>
                    </td>
