...
export default function SettingsPage() {
  // Restrictive while /auth/me is in flight, and deliberately the opposite of the
  // sidebar's permissive rule. The sidebar can afford to guess because every API
  // behind it refuses; most of the cards here have no such backstop, so a wrong
  // guess would flash the account's Anthropic key, the Assistro credentials and
  // the ingest endpoints at a closer before the answer landed.
  //
  // This page had no check of any kind: every rep on every workspace was being
  // shown the whole operator console.
  var access = useAccess();
  var canSeeTeam = !!(access && access.canSeeTeam);
  var isOperator = !!(access && (access.isOperator || access.isOwner));
  ...
  return (
    <div className="p-6 max-w-[1200px] mx-auto">
      ...
      <div className="space-y-6">
        ...
        {/* Their own workspace's look and the material their reps are handed. */}
        {canSeeTeam && (
          <>
            <ReportBrandingCard />
        ...
        {/* Keys, endpoints, the Assistro connection and anything install-wide.
            None of it is a rep's business and some of it is a credential. */}
        {isOperator && (
          <>
            <AiKeyCard />
          {/* ===== LIVE STATUS ===== */}
