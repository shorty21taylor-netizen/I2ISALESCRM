...
import { Users, User, ChevronLeft, ChevronRight, TrendingUp, TrendingDown, AlertTriangle, Lightbulb, Brain } from 'lucide-react';
              ...
              return (
                <div key={m.label} className={'glass-card p-5 stagger-' + (idx + 1)}>
                  ...
                  {m.trend !== 0 ? (
                    ...
                    <div className="text-xs font-mono text-crm-muted">No trend data</div>
                  )}
                </div>
              );
            })}
            ...
            {closerPerformances.length === 0 ? (
              ...
              <>
                <p className="text-sm text-crm-text leading-relaxed mb-4">
                  Waiting for enough performance data to generate a team diagnostic report.
                </p>
                <h4 className="text-xs font-mono text-crm-muted uppercase tracking-wider mb-2">Priority Actions</h4>
                <p className="text-sm text-crm-muted">No actions available yet.</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {closerPerformances.length === 0 ? (
            <div className="glass-card overflow-hidden">
              <EmptyState icon={User} title="No individual reports yet" subtitle="Reports generate when closers have enough call and performance data" />
            </div>
          ) : (
