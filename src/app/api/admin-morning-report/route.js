...
export async function POST(req) {
  ...
  try {
    ...
    // If generateOnly, return the message without sending
    if (body.generateOnly) {
      return NextResponse.json({ success: true, message: msg, generateOnly: true });
    }

    // Only send via Assistro if explicitly requested (body.groupId present)
    // When called without groupId, just return the message content for the caller to send
    if (body.groupId && body.assistroApiUrl) {
      try {
        var sendRes = await fetch(body.assistroApiUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': body.assistroApiKey || '',
            'Authorization': body.assistroApiKey ? ('Bearer ' + body.assistroApiKey) : '',
          },
          body: JSON.stringify({ phone: body.groupId, body: msg, type: 2 }),
        });
        var sendStatus = sendRes.status;
        var sendTxt = await sendRes.text();

        // Retry with chatId+message format if 422
        if (sendStatus === 422) {
          var retry = await fetch(body.assistroApiUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-api-key': body.assistroApiKey || '' },
            body: JSON.stringify({ chatId: body.groupId, message: msg }),
          });
          sendStatus = retry.status;
          sendTxt = await retry.text();
        }

        console.log('[Admin Report] Sent to group', body.groupId.substring(0, 20), 'status:', sendStatus);
      } catch (sendErr) {
